...
fn ground_accelerate(velocity: &mut Velocity, wish_dir: Vec3, wish_speed: f32, dt: f32) {
    let current_speed = (wish_dir * wish_speed).dot(**velocity);
    let add_speed = wish_speed - current_speed;
    if add_speed <= 0.0 {
        return;
    }
    let acceleration_speed = (GROUND_ACCELERATION * dt * wish_speed).min(add_speed);
