use crate::{
    common::{input, physics::{col_layers, prelude::*}},
    prelude::*,
};
...
impl Setting for PlayerGroundSettings {
    ...
    fn setting(&self) -> Self::Property {
        Self::Property {
            // Make the player at max_speed after time_to_max_speed elapsed since starting to move.
            acceleration: self.max_speed / self.time_to_max_speed,
            // Decay coeficient for the friction that stops the player in time_to_stop at max_speed.
            friction: f32::ln(self.max_speed / self.min_speed) / self.time_to_stop,
            // The maximum slope angle the player can stand on. If slope exceed this angle the player
            // starts to slip.
